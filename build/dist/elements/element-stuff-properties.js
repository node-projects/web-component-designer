...
let ElementProperties = class ElementProperties extends ElementStuffBase(PolymerElement) {
  static get template() {
    return html`
  ...
  display(target) {
    ...
    this.$.bonus.innerHTML = ''; // If this is the top level div, we don't want any other properties shown.
  ...
  _displayBasicProperties(target) {
    for (let i = 0; i < this._stuff.length; i++) {
      ...
      let el = this.root.querySelector(`[name=${prop}]`); // Ugh
  ...
  _displayPropsOrAttributes(target, propNames, isAttribute = false) {
    for (let i = 0; i < propNames.length; i++) {
      let name = propNames[i]; // Ignore things that start with a _. Polymer converts every property
      ...
      label.setAttribute('for', name); // label.htmlFor = name;
