...
let ActionHistory = class ActionHistory extends PolymerElement {
  constructor() {
    super();
    this.undoHistory = [];
    this.redoHistory = [];
  ...
  undo() {
    ...
    switch (item.action) {
      ...
      case 'new':
        this.dispatchEvent(new CustomEvent('remove-from-canvas', {
          bubbles: true,
          composed: true,
          detail: {
            target: item.node,
            parent: item.detail.parent,
            node: this
          }
        }));
        break;
  ...
  redo() {
    ...
    switch (item.action) {
      ...
      case ActionHistoryType.New:
        this.dispatchEvent(new CustomEvent('add-to-canvas', {
          bubbles: true,
          composed: true,
          detail: {
            target: item.node,
  ...
  updateButtons() {
    this.dispatchEvent(new CustomEvent('update-action-buttons', {
      bubbles: true,
      composed: true,
      detail: {
        undos: this.undoHistory.length,
        redos: this.redoHistory.length,
        node: this
      }
    }));
  }
...
__decorate([property({
  type: Array
})], ActionHistory.prototype, "undoHistory", void 0);
